...
//IndexGet renderea al index de GrupoAlmacen
func IndexGet(ctx *iris.Context) {

	var Send GrupoAlmacenModel.SGrupoAlmacen
	...
	numeroRegistros = GrupoAlmacenModel.CountAll()
	paginasTotales = MoGeneral.Totalpaginas(numeroRegistros, limitePorPagina)
	GrupoAlmacens := GrupoAlmacenModel.GetAll()
	...
	if numeroRegistros <= limitePorPagina {
		Cabecera, Cuerpo = GrupoAlmacenModel.GeneraTemplatesBusqueda(GrupoAlmacens[0:numeroRegistros])
	} else if numeroRegistros >= limitePorPagina {
		Cabecera, Cuerpo = GrupoAlmacenModel.GeneraTemplatesBusqueda(GrupoAlmacens[0:limitePorPagina])
	}
	...
	ctx.Render("GrupoAlmacenIndex.html", Send)

}

//IndexPost regresa la peticon post que se hizo desde el index de GrupoAlmacen
func IndexPost(ctx *iris.Context) {
	...
	//Send.GrupoAlmacen.EVARIABLEGrupoAlmacen.VARIABLE = cadenaBusqueda    //Variable a autilizar para regresar la cadena de búsqueda.

	if cadenaBusqueda != "" {

		docs := GrupoAlmacenModel.BuscarEnElastic(cadenaBusqueda)
		...
		if docs.Hits.TotalHits > 0 {
			...
			Cabecera, Cuerpo := GrupoAlmacenModel.GeneraTemplatesBusqueda(GrupoAlmacenModel.GetEspecifics(arrToMongo))
			Send.SIndex.SCabecera = template.HTML(Cabecera)
			Send.SIndex.SBody = template.HTML(Cuerpo)
			...
			if numeroRegistros <= limitePorPagina {
				Cabecera, Cuerpo = GrupoAlmacenModel.GeneraTemplatesBusqueda(GrupoAlmacenModel.GetEspecifics(arrIDMgo[0:numeroRegistros]))
			} else if numeroRegistros >= limitePorPagina {
				Cabecera, Cuerpo = GrupoAlmacenModel.GeneraTemplatesBusqueda(GrupoAlmacenModel.GetEspecifics(arrIDMgo[0:limitePorPagina]))
			}
...
//AltaGet renderea al alta de GrupoAlmacen
func AltaGet(ctx *iris.Context) {
	...
	ctx.Render("GrupoAlmacenAlta.html", Send)

}

//AltaPost regresa la petición post que se hizo desde el alta de GrupoAlmacen
...
//EditaGet renderea a la edición de GrupoAlmacen
func EditaGet(ctx *iris.Context) {
	...
	ctx.Render("GrupoAlmacenEdita.html", Send)

}

//EditaPost regresa el resultado de la petición post generada desde la edición de GrupoAlmacen
...
func DetalleGet(ctx *iris.Context) {
	...
	ctx.Render("GrupoAlmacenDetalle.html", Send)
}
...
func MuestraIndexPorGrupo(ctx *iris.Context) {
	...
	cadenaBusqueda = ctx.FormValue("searchbox")
	//Send.GrupoAlmacen.ENombreGrupoAlmacen.Nombre = cadenaBusqueda
