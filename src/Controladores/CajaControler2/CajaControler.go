...
func AltaGet(ctx *iris.Context) {
	...
	name, nivel, id := Session.GetUserName(ctx.Request)
	scaja.SSesion.Name = name
	scaja.SSesion.Nivel = nivel
	scaja.SSesion.IDS = id

	if name == "" {
		http.Redirect(ctx.ResponseWriter, ctx.Request, "/Login", 302)
	} else if nivel == "Administrador" {
		scaja.SSesion.IsAdmin = true
		...
		ctx.Render("IndexDashboard.html", scaja)
	}
}
...
func GetSQLHeader(NumMov string, conn *sql.DB) {
